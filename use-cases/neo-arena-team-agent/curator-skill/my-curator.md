# My Curator — second brain playbook (opencode edition)

> **What this file is.** This is the opencode-adapted version of The Curator's
> official agent skill. The original skill ships in Claude's Agent-Skill format
> (`~/.claude/skills/my-curator/SKILL.md`), which opencode does not read. This
> file carries the **same playbook**, reformatted so opencode can load it as a
> plain instructions file.
>
> **How opencode loads it.** opencode has no `~/.claude/skills` folder. Instead,
> drop this file into your agent project and register it in **opencode.jsonc**
> so opencode always reads it:
>
> ```json
> {
...
> ```
>
> (opencode also reads `AGENTS.md` automatically, so an alternative is a line in
> AGENTS.md: *"Before using any my-curator tool, read curator-skill/my-curator.md."*)
>
> **Tool names differ from Claude.** In Claude the tools are named
> `mcp__my-curator__list_domains`, etc. In opencode the same tools are exposed
> under the MCP server you registered — typically `my-curator_list_domains`,
> `my-curator_get_index`, `my-curator_compile_to_wiki`, and so on. Throughout
> this playbook the tools are referred to by their **bare function names**
> (`list_domains`, `get_index`, `compile_to_wiki`, …); use whichever prefixed
> form your opencode build shows in its tool list.
>
> Source of truth (Claude original):
> https://github.com/talirezun/the-curator/blob/main/claude-skills/my-curator/SKILL.md
...
Some domains in `list_domains` may be **Shared Brain mirrors** — named like `shared-<slug>` (e.g. `shared-cohort`, `shared-team`). These are local read-only copies of a collective wiki the user contributes to as part of a cohort, team, or research group.
...
**Writing to a mirror is refused.** All seven write tools (`compile_to_wiki`, `fix_wiki_issue`, `scan_semantic_duplicates` merge path, `dismiss_wiki_issue`, `undismiss_wiki_issue`) check the target domain's config for `readonly: true`. If true, they refuse with this exact error:
...
```
What you (the agent via MCP) do     Where it happens     What it does
─────────────────────────────       ──────────────       ────────────────────
...
Persistent dismissals: `dismiss_wiki_issue` writes to a file synced across the user's machines. Items dismissed in a conversation also disappear from the in-app Health tab; same store. Use `get_health_dismissed` to list previously skipped issues if the user asks.
...
For sample dialogues that show end-to-end flows for each scenario, see the original examples file:
https://github.com/talirezun/the-curator/blob/main/claude-skills/my-curator/examples.md

## §10 — Version compatibility

**This skill targets Curator v3.0.0-beta.1 and later.** The features covered:
...
**Updating this file**: re-download it from the conduit-agent repo (it overwrites the old copy in your project). opencode re-reads it at the start of the next session — no restart of a running conversation is required.
